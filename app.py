...
@st.cache_resource
def load_model():
    """Load the pre-trained model and encoder from disk."""
...
try:
    ...
    st.info("Please ensure 'catalyst_model.pkl' and 'metal_encoder.pkl' are in the project directory.")
    st.stop()
...
st.markdown(
    ...
    Adjust the input parameters below and click **Predict Activity** to get a prediction.
    """
)
...
# Prediction button
if st.button("[PREDICT] Predict Activity", type="primary", use_container_width=True):
    try:
        # Encode the metal type
        ...
        # Display result
        st.success(f"### Predicted Activity: {prediction[0]:.2f}%")
